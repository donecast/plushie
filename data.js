...
data.adminListUsers = async function () {
  ...
  // profiles+summary path if migration 0042 hasn't been applied yet.
  try {
    ...
    console.warn('[adminListUsers] admin_user_overview missing; run migration 0042. Falling back.');
    const { data: rows, error } = await sb
      .from('profiles')
...
// ─── Real names (db/0042: profile_private + security-definer accessors) ──
