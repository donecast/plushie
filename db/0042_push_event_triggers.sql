-- ============================================================
-- 0042 — Fire real (closed-app) push on social + trade events.
--
-- Supersedes 0041_friend_push_trigger.sql, which shipped only the
-- friendship piece. This file captures the FULL push surface that is
-- already live in production (it was hand-applied ahead of merge as
-- "0041_push_event_triggers"); committing it here keeps db/*.sql in sync
-- with prod so a fresh rebuild gets every trigger, not just friendships.
--
