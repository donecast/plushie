-- 0085: trade offering honesty — per-offering includes checklist.
--
...
comment on column trade_items.extras is
  'Includes checklist: [{"name": text, "included": bool}] answered by the owner when listing; null = pre-0085 listing or no expected extras.';
...
begin
  if exists (
    ...
    raise notice '✅ 0085 applied — trade_items.extras is live.';
  else
    raise exception '❌ 0085 failed — trade_items.extras is missing.';
  end if;
end $$;
